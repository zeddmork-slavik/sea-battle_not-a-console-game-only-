...
void compose_frame(GameState* game, double delta_time, Uint32 current_time, const GraphicsContext* ctx,
    ...
    if (!game->player_cannon.is_animating && game->player_cannon.animation_end_time != 0) {
        if (current_time - game->player_cannon.animation_end_time >= game->player_cannon.fire_delay) {
            game->player_cannon.is_firing = 1;
            play_cannon_shot(game);
...
void draw_all_elements(GameState* game, Uint32 current_time, const GraphicsContext* ctx,
    ...
    draw_island(ctx, landmarks->player_x + ctx->field_size + OFFSET_X_FROM_BOARD,
                landmarks->offset_y + OFFSET_Y_FROM_BOARD + ISLAND_BELOW_PLAYER_CANON, IS_PLAYER);
    draw_island(ctx, landmarks->player_x + ctx->field_size + OFFSET_X_FROM_BOARD + X_CRUTCH_COMPUTER_ISLAND,
                landmarks->offset_y + OFFSET_Y_FROM_COMPUTER_CANON + ISLAND_BELOW_PLAYER_CANON +
                    Y_CRUTCH_COMPUTER_ISLAND,
                IS_COMPUTER);
