...
WindowConfig create_default_config(void) { return (WindowConfig){WIDTH, HEIGHT}; }

SDL_Window* create_game_window(WindowConfig config) {  // empty
    return SDL_CreateWindow(                           // SDL_Window - непрозрачная структура из SDL,
                              // SDL_CreateWindow создаёт HWND (идентификатор окна для ОС)
                              // экземпляр window заниимает мегабайты видеопамяти, это реальный объект в ОС
...
SDL_Renderer* create_game_renderer(SDL_Window* window) {  // empty

    // Индекс драйвера (-1 = авто) OpenGL - старенький работяга для всех, Direct3D - специально для винды,
    // Vulkan - крутой но не робит на маках SDL_RENDERER_ACCELERATED  определяется аппаратное ускорение -
    // здесь используй видюху(1000 FPS) а не проц(60FPS) или есть VSync - он заставляет видюху ждать
    // обновления кадра на мониторе, но говорят для обучения это too mach но он нужен будет на анимацию
...
SDL_Texture* load_texture_from_file(SDL_Renderer* renderer, const char* filename) {
    ...
    if (surface) {
        // Шаг 4: SDL_CreateTextureFromSurface() - это как перенести картинку в память видеокарты (она быстрее
        // намного)
...
void cleanup_graphics(GraphicsContext* ctx) {  // вызывается из main()
    if (ctx->renderer) {
        SDL_DestroyRenderer(ctx->renderer);  // если напишем после SDL_DestroyWindow - то ничего не будет
                                             // удалено - не найдут его при отсутсвии окна
        ctx->renderer = NULL;
    }
    if (ctx->window) {
        SDL_DestroyWindow(
            ctx->window);  // закрывает окно в ОС, освобождает всю связанную память, аннулирует указатель
        ctx->window = NULL;
    }
}
