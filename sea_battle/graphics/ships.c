...
void draw_all_fires(const GraphicsContext* ctx, GameState* game, const GameLandmarks* landmarks) {
    ...
    while (current != NULL) {
        ...
        switch(current->current_frame) {
            case 0: fire_texture = ctx->fire_ship0; break;
            case 1: fire_texture = ctx->fire_ship1; break;
            case 2: fire_texture = ctx->fire_ship2; break;
            case 3: fire_texture = ctx->fire_ship3; break;
            case 4: fire_texture = ctx->fire_ship4; break;
            case 5: fire_texture = ctx->fire_ship5; break;
            case 6: fire_texture = ctx->fire_ship6; break;
            case 7: fire_texture = ctx->fire_ship7; break;
        }
        
        // Расчёт координат и отрисовка
        SDL_Rect fire_rect = {
            .x = (game->current_turn == IS_COMPUTER ? landmarks->player_x: landmarks->computer_x) + current->cell_x * ctx->cell_size,
