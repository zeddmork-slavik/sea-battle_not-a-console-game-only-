    ...
    if (show_ships) {
        for (char i = 0; i < board->ship_count; i++) {
            ...
            switch (ship.deck_count) {
                case 1:
                    texture = ctx->ship_jup_1p;
                    break;
                case 2:
                    texture = ctx->ship_jup_2p;
                    break;
                case 3:
                    texture = ctx->ship_jup_3p;
                    break;
                case 4:
                    texture = ctx->ship_jup_4p;
                    break;
                default:
                    texture = NULL;
                    break;
            ...
            if (texture) {
                draw_ship(ctx, base_x, base_y, ship.x, ship.y, ship.direction, ship.deck_count, texture);
            }
        }
    }
}
...
void draw_single_grid(const GraphicsContext* ctx, int offset_x, int offset_y) {  // offset - сдвиг
    ...
    for (char x = 0; x <= GRID_SIZE; x++) {
        SDL_RenderDrawLine(ctx->renderer, offset_x + x * ctx->cell_size, offset_y,
                           offset_x + x * ctx->cell_size, offset_y + ctx->field_size);
    }

    // Рисуем горизонтальные линии
    for (char y = 0; y <= GRID_SIZE; y++) {
        SDL_RenderDrawLine(ctx->renderer, offset_x, offset_y + y * ctx->cell_size, offset_x + ctx->field_size,
                           offset_y + y * ctx->cell_size);
    }
}